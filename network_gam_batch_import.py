...
report_job = {
      'reportQuery': {
          'dimensions': ['PLACEMENT_NAME', 'ADVERTISER', ],
...
try: 
  df.to_csv(r'/Users/christian/Documents/Work/gam_api/output_data/line_items_report_network.csv', index=False)
  print('Report downloaded successfully')
except errors.AdManagerReportError as e: 
  print('Failed to download report. Error was: %s' % e)

engine = sqla.create_engine('postgresql://doadmin:patxi1ctdovgrml4@etl-db-do-user-9163711-0.b.db.ondigitalocean.com:25060/network_etl', pool_pre_ping=True)
...
def connect(engine):
    conn = None
    try:
        print('Connecting to the PostgreSQL[network_reporting]....')
        conn = engine
...
dff = pd.read_csv(r'/Users/christian/Documents/Work/gam_api/output_data/line_items_report_network.csv')
...
def copy_from_dataframe(conn, dff):
    ...
    try:
        dff.to_sql('network_export',conn, if_exists='replace')
...
default_args = {
  ...
  'execution_timeout': timedelta(seconds=1000000)
}

dag = DAG(
   'GAM_batch_import_network',
