...
report_job = {
      'reportQuery': {
          'dimensions': ['ORDER_NAME', 'ORDER_ID'],
          'columns': ['AD_SERVER_IMPRESSIONS', 'AD_SERVER_ALL_REVENUE', 'AD_EXCHANGE_LINE_ITEM_LEVEL_REVENUE'],
...
df['Column.AD_SERVER_ALL_REVENUE'] = df['Column.AD_SERVER_ALL_REVENUE'].div(1000000).round(2)
df['Column.AD_EXCHANGE_LINE_ITEM_LEVEL_REVENUE'] = df['Column.AD_EXCHANGE_LINE_ITEM_LEVEL_REVENUE'].div(1000000).round(2)
df.rename(columns={
  'Dimension.ORDER_NAME' : 'Order',
  'Dimension.ORDER_ID' : 'Order_ID',
  'Column.AD_SERVER_IMPRESSIONs' : 'Ad_server_impressions',
  'Column.AD_SERVER_ALL_REVENUE' : 'Ad_server_CPM_CPC_CPD_and_vCPM_revenue',
  'Column.AD_EXCHANGE_LINE_ITEM_LEVEL_REVENUE' : 'Ad_exchange_revenue'
}, inplace=True)



try: 
  df.to_csv(r'/Users/christian/Documents/Work/gam_api/output_data/network_forecast.csv', index=False)
...
def connect(engine):
    conn = None
    try:
        print('Connecting to the PostgreSQL[gam_etl.db]....')
        conn = engine
...
dff = pd.read_csv(r'/Users/christian/Documents/Work/gam_api/output_data/network_forecast.csv')
...
def copy_from_dataframe(conn, dff):
    ...
    try:
        dff.to_sql('network_forecast',conn, if_exists='replace')
